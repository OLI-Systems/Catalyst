...
//   <sessionId>.jsonl
// Confirmed on disk, e.g. C:\Source\repos\Catalyst → C--Source-repos-Catalyst
...
// Deletes specific transcripts, identified by conversation id.
//
// Ids are resolved through list() rather than taking a path from the caller, so
// only files this repo + CLI actually owns can ever be removed — a path from the
// client could point anywhere on disk.
//
// Destructive and not recoverable: callers must confirm with the user first.
function remove(cli, repoPath, ids) {
  const wanted = new Set((Array.isArray(ids) ? ids : [ids]).filter(Boolean).map(String));
  if (!wanted.size) return { removed: 0, errors: [] };
  ...
  for (const c of conversations) {
    if (!c.file || !wanted.has(String(c.id))) continue;
  ...
  for (const id of wanted) {
    if (!seen.has(id)) errors.push(`No conversation ${id} for this repository`);
  }
  return { removed, errors };
}

// Deletes every transcript for this repo + CLI.
function clear(cli, repoPath) {
  const { conversations } = list(cli, repoPath);
  return remove(cli, repoPath, conversations.map((c) => c.id));
}

module.exports = { list, remove, clear, claudeProjectDir, geminiProjectName };
