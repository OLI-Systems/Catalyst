...
// Spawn a PTY that runs a CLI command string under the platform shell:
//   Windows → cmd.exe /c <command>; POSIX → /bin/bash -c <command>.
// `env` is optional; defaults to a fresh enrichEnv() so a restart picks up
// current credentials/PATH.
function spawnCliPty(command, repoPath, env) {
...
async function createSession(cli, repoPath, repoName, onData, onExit, worktreeOpts) {
  ...
  const ptyProcess = spawnCliPty(command, repoPath);
  ...
  store.addSession({ id, cli, repo: repoName, repoPath, worktreePath: session.worktreePath, originalRepoPath: session.originalRepoPath });

  return { id, cli, repo: repoName, repoPath, worktreePath: session.worktreePath, worktreeBranch: session.worktreeBranch };
}
...
function restartSession(sessionId) {
  ...
  buf.reconnectParts = [];
  buf.reconnectSize = 0;
  session.ptyProcess = spawnCliPty(session.command, session.repoPath, enrichEnv());
  bindPtyHandlers(session);
  return true;
}
